...
flags.DEFINE_integer('epochs', -1, 'Number of epochs, -1 for infinite')
...
def main(_):
    ...
    while step < FLAGS.batch_size * FLAGS.epochs or FLAGS.epochs == -1:
        if FLAGS.render: env.render()
        ...
        if done:
            ...
            print 'Finished episode %d, updating rewards...' % step
            loss, rewards = policy_net.partial_fit_step(train_batch=update_gradients)

            print 'Episode %d of %d' % (step, FLAGS.batch_size * FLAGS.epochs)
            print '='*40
            print 'Loss = %f' % loss
